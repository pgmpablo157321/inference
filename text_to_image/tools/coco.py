...
def get_args():
    ...
    parser.add_argument(
        "--num-workers", default=1, type=int, help="Path to latents"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--latents-path", default="latents.pt", type=str, help="Path to latents"
    )
...
if __name__ == "__main__":
    ...
    os.system(f"cd {dataset_dir}/download_aux/ && \
              wget http://images.cocodataset.org/annotations/annotations_trainval2014.zip --show-progress")
    
    # Unzip file 
    with zipfile.ZipFile(f"{dataset_dir}/download_aux/annotations_trainval2014.zip", 'r') as zip_ref:
        zip_ref.extractall(f"{dataset_dir}/raw/")
    
    # Move captions to target folder
    os.makedirs(f"{dataset_dir}/captions/", exist_ok=True)
    os.system(f"mv {dataset_dir}/raw/annotations/captions_val2014.json {dataset_dir}/captions/")
    ...
    tasks = [(row["coco_url"], f"{dataset_dir}/validation/data/", row['file_name']) for i, row in df_annotations.iterrows()]
    ...
    if os.path.exists(args.latents_path):
        os.makedirs(f"{dataset_dir}/latents/", exist_ok=True)
        os.system(f"cp {args.latents_path} {dataset_dir}/latents/")
